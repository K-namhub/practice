...
public class Student {  // Student 클래스의 머리부
   ...
   public void setStuName() { // 학생이 이름을 받는 메소드의 머리부
	  Scanner scan = new Scanner(System.in);	// Scanner 객체를 생성하고 scan이 그 객체를 가리키게 함
      System.out.println("학생의 이름을 입력하라.");	// "학생의 이름을 입력하라." 출력
      StuName = scan.nextLine();				// 입력받은 이름을 StuName에 저장
   }   
   ...
   public void setHomework(int Hscore) {	// 과제 점수를 받는 메소드의 머리부
      while(true) {								// 반복문을 무한으로 돌린다
         ...
         if(HomeGrade < 0 || HomeGrade > 100) {			// HomeGrade의 값이 0보다 작거나 100보다 크다면
            ...
            System.out.println(StuName+ "의 과제점수를 다시 입력하라.");	    // StuName"의 과제점수를 다시 입력하라." 출력 후 다시 입력 받음
   ...
   public void setTest(int Tscore) {		// 시험 점수를 받는 메소드의 머리부
      while(true) {								// 반복문을 무한으로 돌린다
         ...
         if(TestGrade < 0 || TestGrade >100) {			// TestGrade의 값이 0보다 작거나 100보다 크다면
            ...
            System.out.println(StuName + "의 시험점수를 다시 입력하라.");	// StuName"의 시험점수를 다시 입력하라." 출력 후 다시 입력 받음
   ...
   public void inputGrades(){	// 학생의 이름, 과제 점수, 시험점수를 받는 메소드의 머리부
	  Scanner scan = new Scanner(System.in);	// Scanner 객체를 생성하고 scan이 그 객체를 가리키게 함
	  System.out.println(StuName + "의 과제점수를 입력하라.");	// StuName"의 과제점수를 입력하라." 출력
      setHomework(scan.nextInt());		// setHomework 메소드를 호출
      System.out.println(StuName + "의 시험점수를 입력하라.");	// StuName"의 시험점수를 입력하라." 출력
   ...
   public double getTotal() {		// 과제와 시험 점수의 총점수를 계산 하는 메소드의 머리부
      return HomeGrade*0.4 + TestGrade*0.6;		// 시험 반영 점수맞게 계산하여 반환해줌
   }
   ...
   public void print() {			// 총 결과를 출력해주는 메소드의 머리부
         System.out.println(StuName + "의 총점은" + getTotal() + "이고 학점은 " + getGrade(getTotal()) + "이다."); // StuName "의 총점은"getTotal()"이고 학점은 "getGrade(getTotal())"이다." 출력해줌
      }
}
